...
const Dropdown = ({ submenus, dropdown }) => {
    return (
        <ul id='dropdown' className={`${dropdown ? 'inline' : 'hidden'} flex flex-col absolute laptop:flex-row`}>
            {submenus.map((submenu, index) => (
                <li key={index}>
                    <Link
                        ...
                        className='text-c-white hover:text-c-blue block px-4 py-2 text-xl animate-nav3-down laptop:animate-nav3-slide active:text-c-yellow'
                    >
                        {submenu.title}
                    </Link>
                </li>
